...
from PySide6.QtWidgets import QFileDialog, QMessageBox, QDialog, QStyledItemDelegate
...
class MainWindow(QtWidgets.QMainWindow, _UI_control.Ui_MainWindow):
    ...
    def selectproject(self):
        ...
        if self.projfolder:
            ...
            for (root, dirs, files) in os.walk(self.projfolder, topdown=True):
                for file in files:
                    # check if cameraname file delimiter is in the filename and add to set
            ...
            # selct row 0 and focus
            self.table_cameras.selectRow(0)
            self.table_cameras.setFocus()
    ...
    def go(self):
        ...
        '''
        if not self.projectselected:
            messagepop('Select project')

        else:
            self.savefolder = QFileDialog.getExistingDirectory(self, 'Select folder to save to',
                                                               '', options=OPTIONS)

            if (self.savefolder and self.projfolder):
                ...
                self.fillxlsx(EVTEXTENSION, EVENTSEPARATOR, 'events')
    ...
    def fillxlsx(self, filetype, separator, descr):
        ...
        if filetype == TRACKEXTENSION:
            '''
            ASFIndex,Date,Time,Area,Lat,Long,SOG,ROV Acc Depth,Heading,mp,Altitude,Sal,T,P ,E,N
            00:00:00.508,28/01/2026,06:27:54,34_T13,39.503348562,15.150617013,0.2,-742.92,302.09,-41.56,72.19,38.83,14.41,742.22,4766158.28,1838688.17
            00:00:01.504,28/01/2026,06:27:55,34_T13,39.503347942,15.150618928,0.2,-743.66,302.47,-41.43,72.19,38.83,14.41,742.94,4766158.45,1838688.11
            '''
        ...
        if filetype == EVTEXTENSION:
            ...
            ;NEW
            start_marker_table
            00:00:00.123	ID,Date,Time,Area,Primary,Secondary,Easting,Northing,Prof,Temp,Sal,Lat,Long,Commento
            00:02:20.161	948,28/01/2026,01:57:38,,Tipo di substrato,Roccia infangata,4767143.37,1840337.78,925.57579,14.27,38.79,39.5176490378,15.1631644070,
            end_marker_table
            '''
            dflist = []
            for f in fileslist:
                ...
                # add full dv filename for 'dv index' file - not added to 'events' file
...
class MergeWindow(QtWidgets.QMainWindow, _UI_merge.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.sp_colto.valueChanged.connect(self.setcolto)
    ...
    def fill_table(self, ft):
        ...
        if ft == 'events':
            ...
            # fill events table
            for row in range(self.table_evts.rowCount()):
                for col in range(self.table_evts.columnCount()):
                    item = QtWidgets.QTableWidgetItem(str(self.evts.iloc[row, col]))
                    # specify editable column
                    if col == EVENTFIELDTOMERGE - 1:
                        # make editable
        ...
        if ft == 'comments':
            '''
            Load 'comments' csv or xlsx file
            '''
        ...
        if ft == 'dvix':
            '''
            Load 'dv index' xls file
            '''
            self.dvix = pd.read_excel(self.dvifnv)
            self._dvinit = self.dvix['FN'][0]
            # make an instance of QMediaPlayer imported from _QtPl module
    ...
    def filter_comments(self, mode):
        '''
        Set filter to 'comments' table
        :param mode: 'setfilter' - set filter; 'resetfilter' - reset filter
        :return:
        '''
    ...
    def setcolto(self):
        '''
        Make 'events' column to merge editable
        '''
        ...
        for row in range(self.table_evts.rowCount()):
            for col in range(self.table_evts.columnCount()):
                item = self.table_evts.item(row, col)
                if col == EVENTFIELDTOMERGE - 1:
                    # make editable
                    item.setFlags(item.flags() | Qt.ItemIsEditable)
                else:
    ...
    def merge(self):
        '''
        Merge 'comments' table entries to 'events' table based on selected columns and separator
        :return:
        '''
        try:
            ...
            if self.evts_selected and self.cmts_selected:
                ...
                for i, ts in enumerate(self.evttimestamps):
                    ...
                    if minvar <= self.sp_maxvar.value():
                        ...
                        # grab original event entry
                        event = self.table_evts.item(i, EVENTFIELDTOMERGE - 1).text()
                        # add separator if event and comment not empty
                        event = event if (event == '' or comment == '') else event + (' ' + FIELDSEPARATOR + ' ')

                        comment = event + comment if self.rb_merge.isChecked() else comment
    ...
    def save_events(self):
        '''
        Save merged 'events' to Excel file in the same folder as original 'events' file
        :return:
        '''
    ...
    def getclickedevt(self, row, column):
        '''
        iv 'dv index' selected, load and navigate through DV
        :param row: 'events' table row
        :param column: 'events' table column
        :return:
        '''
...
def main():
    ...
    global TRACKEXTENSION               # extension of VisualSoft track files (def: '.csv')
    global EVTEXTENSION                 # extension of VisualSoft event files (def: '.evt')
    global DVEXTENSION                  # extension of created dv index ('.xlsx')
    global CAMDELIMITER                 # VisualSoft cameraname file delimiter (def: '@')
    global TRACKSEPARATOR               # fields separator in VisualSoft track files (def: ',')
    global EVENTSEPARATOR               # fields separator in VisualSoft event files (def: ',')
    global COMMENTSEPARATOR             # fields separator in ISPRA csv comments (def: ';')
    global MAXVARIATION                 # max time difference to mergecomments to events in seconds  (def: 20; max 9999)
    global EVTDATEFIELD                 # concatenated 'events' file DATE field (def: 0)
    global EVTTIMEFIELD                 # concatenated 'events' file TIME field (def: 0)
    global EVTDTFORMAT                  # concatenated 'events' file DATETIME format list (def: ["%d/%m/%Y", "%H:%M:%S"])
    global CMTDATEFIELD                 # concatenated 'comments' file DATE field (def: 0)
    global CMTTIMEFIELD                 # concatenated 'comments' file TIME field (def: 0)
    global CMTDTFORMAT                  # concatenated 'comments' file DATETIME format list (def: ["%d/%m/%Y", "%H:%M:%S"])
    global ASFDTFROMAT                  # concatenated 'dv index' file ASF format (def: "%H:%M:%S.%f")
    global MAKEDVI                      # create 'dv index' file 0/1 (def: 1)
    global COMMENTFIELDTOMERGE          # fields to merge from 'comments' file list (def: [6, 7])
    global EVENTFIELDTOMERGE            # field to merge to 'events' file (def: 13)
    global FIELDSEPARATOR               # merged fields text separator (def: '/')


    file = sys.argv[0]
    # executable parent folder and path to cfg.json
